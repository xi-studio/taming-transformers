...
class Net2NetTransformer(pl.LightningModule):
    ...
    def log_images(self, batch, temperature=None, top_k=None, callback=None, lr_interface=False, **kwargs):
        ...
        N = 4
        if lr_interface:
            x, c1, c2 = self.get_xc(batch, N, diffuse=False, upsample_factor=8)
        else:
            x, c1, c2 = self.get_xc(batch, N)
        x = x.to(device=self.device)
        c1 = c1.to(device=self.device)
        c2 = c2.to(device=self.device)

        quant_z, z_indices = self.encode_to_z(x)
        quant_c1, c1_indices = self.encode_to_c(c1)
        quant_c2, c2_indices = self.encode_to_c(c2)


        # sample
        z_start_indices = z_indices[:, :0]
        index_sample = self.sample(z_start_indices, c1_indices,
                                   c2_indices,
